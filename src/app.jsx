import {KEY as datasetKey, SELECTION_KEY} from './datasets';
...
const links = [
  ...
  ['statistics','statistics/','Statistics',ChartNoAxesCombined],
  ['my-data','my-data/','My data & backups',HardDrive]
];
...
function Home({runs,banks}) {
  ...
  return <>
    ...
    <div className="metrics"><Metric label="Practice questions" value={banks.reduce((n,b)=>n+b.questions.length,0)} detail={t('Foundation and Practitioner')} icon={Layers}/><Metric label="Questions answered" value={latest.length} detail={t('Latest responses, across both parts')} icon={CheckCheck}/><Metric label="Current accuracy" value={max?`${percent(earned,max)}%`:'-'} detail={t('Latest earned points / possible points')} icon={Target}/><Metric label="Completed sessions" value={runs.length} detail={window.TogafCloud?'Account history; check sync status':'Saved on this browser'} icon={HardDrive}/></div>
...
function App() {
  ...
  useEffect(()=>{const listener=e=>{if(e.key===stats.KEY||e.key===datasetKey||e.key===SELECTION_KEY||e.key===null) refresh();};window.addEventListener('storage',listener);window.addEventListener('cloudchange',refresh);return()=>{window.removeEventListener('storage',listener);window.removeEventListener('cloudchange',refresh);};},[]);
  ...
  return <div className="app-shell" onClickCapture={e=>{
    ...
    <main id="main" tabIndex={-1}><Account/>{page!=='my-data'&&!selectBanks().length&&<Notice>No question sets enabled. <a href={href('my-data/#datasets')}>Enable sets in My data</a>.</Notice>}{datasetError&&<Notice error>{datasetError}</Notice>}{record.error&&<Notice error>{record.error} <a href={href('my-data/#backup')}>{t('Manage backups')}</a></Notice>}
      {page==='home'&&<Home runs={selectRuns(record.runs)} banks={selectBanks()}/>}{page==='practice'&&<Practice runs={selectRuns(record.runs)} refresh={refresh} setDirty={setDirty} confirm={confirm}/>}{page==='statistics'&&<Statistics runs={record.runs}/>} {page==='my-data'&&<ProgressPage runs={record.runs} refresh={refresh} confirm={confirm}/>}
    </main></div>
