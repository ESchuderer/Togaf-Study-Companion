...
import {stats,banks,t,href,score,best,percent,time,prepare,resultOf,download} from '../model';
...
function defaults() {
  ...
  return {part,count:[0,2,8,10,20,40].includes(Number(p.get('count')))&&p.has('count')?Number(p.get('count')):part===1?20:8,
...
export function Practice({runs,refresh,setDirty,confirm}) {
  ...
  const questions=banks.filter(b=>b.part===config.part).flatMap(b=>b.questions);
