...
import {PageHeading,LinkButton,Notice,Check as Checkbox,Field} from '../components/shared';
import {stats,selectBanks,t,href,score,best,percent,time,prepare,resultOf,download} from '../model';

function defaults(customOnly) {
  const banks=selectBanks(customOnly);
  ...
  return {part,customOnly,source:banks.some(b=>b.part===part&&b.src===p.get('dataset'))?p.get('dataset'):'',count:[0,2,8,10,20,40].includes(Number(p.get('count')))&&p.has('count')?Number(p.get('count')):part===1?20:8,
    mode:p.get('mode')==='exam'?'exam':'study',onlyMissed:p.get('missed')==='1',unseen:true,
    topics:new Set(all.map(q=>q.topic).filter(topic=>!requested||requested.includes(topic)))};
}

export function Practice({runs,refresh,setDirty,confirm,customOnly=false}) {
  const banks=selectBanks(customOnly);
  const [config,setConfig]=useState(()=>defaults(customOnly)),[search,setSearch]=useState(''),[error,setError]=useState('');
  const [session,setSession]=useState(()=>{
    if(new URLSearchParams(location.search).get('auto')!=='1')return null;
    try{return prepare(defaults(customOnly),runs);}catch{return null;}
  });
  ...
  const questions=banks.filter(b=>b.part===config.part&&(!config.source||b.src===config.source)).flatMap(b=>b.questions);
  const topics=[...new Set(questions.map(q=>q.topic))].sort((a,b)=>t(a).localeCompare(t(b)));
  const latest=stats.latest(config.part,runs),missed=new Set(latest.filter(q=>q.earned<q.max).map(q=>q.id));
  const topicStats=new Map(stats.topics(config.part,runs).map(q=>[q.topic,q]));
  const matches=questions.filter(q=>config.topics.has(q.topic)&&(!config.onlyMissed||missed.has(stats.qid(q)))).length;
  ...
  if(result) return <>
    ...
    <Card id="result" className="result-card"><CardContent><div className="result-score"><strong>{percent(result.score,result.max)}<small>%</small></strong><div><Badge variant={result.pass?'default':'secondary'}>{t(result.pass?'Practice threshold reached':'Keep practicing')}</Badge><p id="score-line">{result.score} / {result.max} {t('points')}</p></div></div><div className="result-meta"><span><Clock size={16}/>{time(result.ms)}</span><span>{t('Part {part}',{part:result.part})}</span><span>{t(result.mode==='study'?'Study mode':'Exam mode')}</span><span role="status">{t(saved?'Saved on this browser':'Not saved')}</span></div><p className="muted">{t('Practice threshold: 60%. Study-mode scores and repeated questions do not establish exam readiness.')}</p></CardContent><CardFooter className="gap-2 flex-wrap"><Button id="review-btn" onClick={()=>setReview(!review)}>{t(review?'Hide answer review':'Review all answers')}</Button><LinkButton to="my-data/" variant="outline">{t('My data & backups')}<ArrowRight/></LinkButton></CardFooter></Card>
  ...
  return <><PageHeading eyebrow="Build your practice" title="Practice with purpose.">{t('Choose a part, narrow your topics and work through a fresh mix.')}</PageHeading>{error&&<Notice error>{error}</Notice>}
    <div className="config-grid"><Card><CardHeader><CardTitle>{t('Session settings')}</CardTitle></CardHeader><CardContent className="space-y-6"><fieldset><legend>{t('Part')}</legend><div className="segmented">{[1,2].map(part=><label key={part} className={config.part===part?'chosen':''}><input type="radio" name="part" value={part} checked={config.part===part} onChange={()=>{setSearch('');update({part,source:'',count:part===1?20:8,topics:new Set(banks.filter(b=>b.part===part).flatMap(b=>b.questions.map(q=>q.topic)))});}}/>{t(part===1?'Part 1: Foundation':'Part 2: Practitioner')}</label>)}</div></fieldset><Field id="dataset" label="Question set"><select id="dataset" value={config.source} onChange={e=>update({source:e.target.value,topics:new Set(banks.filter(b=>b.part===config.part&&(!e.target.value||b.src===e.target.value)).flatMap(b=>b.questions.map(q=>q.topic)))})}><option value="">{customOnly?'All custom sets':'All sets'}</option>{banks.filter(b=>b.part===config.part).map(b=><option key={b.src} value={b.src}>{b.title}</option>)}</select></Field><Field id="count" label="Session length"><select id="count" value={config.count} onChange={e=>update({count:Number(e.target.value)})}>{[2,8,10,20,40,0].map(n=><option key={n} value={n}>{t(n?'{count} questions':'All matching',{count:n})}</option>)}</select></Field><Mode value={config.mode} onChange={mode=>update({mode})}/><div className="space-y-3"><Checkbox id="unseen-first" checked={config.unseen} onChange={e=>update({unseen:e.target.checked})}>{t('Prefer questions I have not seen yet')}</Checkbox><Checkbox id="only-missed" checked={config.onlyMissed} onChange={e=>update({onlyMissed:e.target.checked})}>{t('Only questions below full credit on my latest answer')}</Checkbox></div></CardContent><CardFooter className="config-submit"><p id="match-count" role="status">{t('{count} questions match',{count:matches})}</p><Button id="build-btn" disabled={!matches} onClick={prepareSession}>{t('Prepare session')}<ArrowRight/></Button></CardFooter></Card>
...
function Question({q,part,choice,reveal,onSelect,index,total}) {
  ...
  return <Card className="question-card" data-question-id={stats.qid(q)}><CardHeader><div className="question-meta"><Badge variant="secondary">{t(q.topic)}</Badge><span>{q.lo}</span><span>{index+1} / {total}</span></div>{part===2&&<div className="scenario"><h2>{shown.stitle}</h2><p>{shown.scenario}</p></div>}<h2 className="question-text">{shown.q}</h2>{shown.diagramDescription&&<p>{shown.diagramDescription}</p>}{shown.images?.map((src,i)=><img key={i} src={src} alt={`Question diagram ${i+1}`} className="max-w-full h-auto rounded bg-white"/>)}</CardHeader><CardContent><div className="answer-options">{q.keys.map(key=><button key={key} type="button" data-key={key} aria-pressed={choice===key} disabled={!onSelect||reveal} className={'answer-option '+(choice===key?'selected ':'')+(reveal&&key===bestKey?'correct ':reveal&&choice===key?'incorrect ':'')} onClick={()=>onSelect?.(key)}><span className="option-letter">{key}</span><span>{options[key]}{reveal&&(key===bestKey||key===choice)&&<small className="option-status">{t(key===bestKey?'Best answer':'Your answer')}{part===2?` · ${q.pts[key]} / 5`:''}</small>}</span></button>)}</div>{reveal&&<div className="rationale"><h3>{t('Why this answer?')}</h3>{shown.note&&<p><strong>Review note: </strong>{shown.note}</p>}<p>{t('Your score')}: {score(part,q,choice)} / {part===1?1:5}{!choice?' · '+t('Unanswered'):''}</p>{part===1?<p>{shown.e}</p>:q.keys.map(key=><p key={key}><strong>{key}. </strong>{shown.rat[key]}</p>)}</div>}</CardContent></Card>;
}
